...
STATED_OBJECTIVE = (
    "Optimize Battery Life: keep agents charged, prioritize safe charging decisions, "
    "and never let any agent go below the critical battery threshold."
)
...
SHARED_CONTEXT_TEXT_TEMPLATE = (
    ...
    "Objective battery rule: critical means battery <30. Plan early enough that no agent drops below 30 charge.\n"
    "Basic summary:\n"
    "- Multi-agent warehouse with AGV and Picker collaboration.\n"
    "- Focus on battery life for every agent.\n"
    "- Avoid any out-of-charge scenario and never go below critical charge.\n"
    "- Use charging strategically before battery reaches critical.\n"
...
AGV_JSON_TEMPLATE = (
    "Main objective: Optimize Battery Life: keep agents charged, prioritize safe charging decisions, and never let any agent go below the critical battery threshold.\n"
    "Objective battery rule: critical means battery <30. Plan early enough that no agent drops below 30 charge.\n"
    "Basic summary:\n"
    "- Multi-agent warehouse with AGV and Picker collaboration.\n"
    "- Focus on battery life for every agent.\n"
...
CENTRAL_LANGUAGE_TEMPLATE = (
    "Main objective: {stated_objective}\n"
    "Objective battery rule: critical means battery <30. Plan early enough that no agent drops below 30 charge.\n"
    "You are one centralized planner for the Objective 3 warehouse task.\n"
...
parser = ArgumentParser(
    description="Run objective-3 centralized LLM experiments across models and warehouse scenarios",
...
parser.add_argument("--results_dir", type=str, default="results/obj3_centralized_llm_experiments", help="Directory for logs and summaries")
...
def obj3_candidate_action_ids(env, agent_idx: int, valid_masks: np.ndarray) -> List[int]:
    agent = env.agents[agent_idx]
...
def obj3_candidate_action_ids_with_support(
    env,
    agent_idx: int,
    valid_masks: np.ndarray,
    persisted_actions: List[int] | None = None,
    ...
    return obj3_candidate_action_ids(env, agent_idx, valid_masks)
...
def build_candidate_views(
    ...
    agent = env.agents[agent_idx]
    candidate_ids = obj3_candidate_action_ids_with_support(
        env,
        agent_idx,
        valid_masks,
        persisted_actions,
...
def battery_objective_snapshot(agent_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def run_single_episode(
    ...
    while True:
        ...
        battery_step_objective = battery_objective_snapshot(post_step_agents)
        below_required_min_charge_events += int(battery_step_objective["below_required_min_charge_count"])
        ...
        step_records.append(
            {
                ...
                "battery_objective": battery_step_objective,
    ...
    summary = {
        ...
        "battery_objective_success": int(below_required_min_charge_events) == 0,
    ...
    summary_text_lines = [
        ...
        f"Critical battery threshold: {summary['critical_battery_threshold']:.1f}",
        f"Minimum required charge: {summary['minimum_required_charge']:.1f}",
        ...
        f"Battery objective success: {summary['battery_objective_success']}",
    ...
    log(
        ...
        f"battery_objective_success={summary['battery_objective_success']}",
        log_path,
    )
    return summary
