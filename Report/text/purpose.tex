...
This thesis investigates \gls{llm}-based control as an alternative to purely hand-coded heuristics or fully learned \gls{marl} policies.
...
\begin{enumerate}
    ...
    \item Compare different \gls{llm} sizes/architectures and prompt designs against a heuristic baseline (and, where available, learned \gls{rl} baselines).
...
\begin{itemize}
    ...
    \item The present results focus on small-scale environments (``tiny'') to enable rapid iteration on prompting and validation before scaling to larger warehouses and more agents.
\end{itemize}
