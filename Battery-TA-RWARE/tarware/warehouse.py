...
class Warehouse(gym.Env):
    ...
    def attribute_macro_actions(self, macro_actions: List[int]) -> Tuple[int, int]:
        ...
        charging_cells = {(station.x, station.y) for station in self.charging_stations}
            ...
            if (
                ...
                if agent.type == AgentType.AGV and picker_id:
                    self._event_log(
                        ...
                        f"shelf_id={shelf_id} pos=({agent.x},{agent.y})"
                    )
    ...
    def _execute_unload(self, agent: Agent, rewards: np.ndarray[Any, np.dtype[np.float64]]) -> np.ndarray[Any, np.dtype[np.float64]]:
        ...
        if not self._is_highway(agent.x, agent.y):
            if (
                ...
                carried_id = agent.carrying_shelf.id
                ...
                if agent.type == AgentType.AGV and picker_id:
                    self._event_log(
                        f"[COLLAB] step={self._cur_steps} event=UNLOAD agv_id={agent.id} picker_id={picker_id} "
                        f"shelf_id={carried_id} pos=({agent.x},{agent.y})"
                    )
    ...
    def process_shelf_deliveries(self, rewards: np.ndarray[Any, np.dtype[np.float64]]) -> Tuple[np.ndarray[Any, np.dtype[np.float64]], int]:
        ...
        for y, x in self.goals:
            ...
            if not agent.has_delivered:
                ...
                self._event_log(
                    f"[DELIVERY] step={self._cur_steps} agv_id={agent.id} shelf_id={shelf_id} "
                    f"goal=({y},{x}) queue_remaining={len(self.request_queue)}"
                )
            shelf_deliveries += 1
