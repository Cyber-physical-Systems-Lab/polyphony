...
STATED_OBJECTIVE = (
    "Maximize total shelf deliveries while keeping agents charged enough to avoid critical battery levels."
)
CRITICAL_BATTERY_THRESHOLD = 30.0
MINIMUM_REQUIRED_CHARGE = CRITICAL_BATTERY_THRESHOLD
MIN_AGV_UNLOAD_WAIT_STEPS = 5


SHARED_CONTEXT_TEXT_TEMPLATE = (
    "Main objective: {stated_objective}\n"
    "Safety constraint: keep agents at or above 30 charge whenever possible; plan charging before they become critical.\n"
    "Basic summary:\n"
    "- Multi-agent warehouse with AGV and Picker collaboration.\n"
    "- Avoid any out-of-charge scenario and plan charging before agents become critical.\n"
    "- Use charging strategically so delivery throughput does not collapse.\n"
    ...
    "Your output should prioritize valid AGV-Picker delivery cycles while keeping agents above critical battery levels.\n"
)
...
AGV_JSON_TEMPLATE = (
    "Main objective: Maximize total shelf deliveries while keeping agents charged enough to avoid critical battery levels.\n"
    "Safety constraint: keep agents at or above 30 charge whenever possible; plan charging before they become critical.\n"
    "Basic summary:\n"
    "- Multi-agent warehouse with AGV and Picker collaboration.\n"
    "- Avoid any out-of-charge scenario and plan charging before agents become critical.\n"
...
CENTRAL_LANGUAGE_TEMPLATE = (
    "Main objective: {stated_objective}\n"
    "Safety constraint: keep agents at or above 30 charge whenever possible; plan charging before they become critical.\n"
    "You are one centralized planner for the Objective 1 warehouse task.\n"
...
parser = ArgumentParser(
    description="Run objective-1 centralized LLM experiments across models and warehouse scenarios",
...
parser.add_argument("--results_dir", type=str, default="results/obj1_centralized_llm_experiments", help="Directory for logs and summaries")
...
parser.add_argument("--only_model", type=str, default=None, help="Run only this model name; kept as an alias for --selected_models")
parser.add_argument("--selected_models", nargs="+", default=None, help="Run only these model names; accepts space-separated names or comma-separated groups")
...
def _fetch_model_max_context(model: str, ollama_url: str, timeout_s: int) -> int:
    ...
    req = request.Request(
        show_url,
        data=str.encode(json.dumps({"name": model})),
...
def selected_model_indices(selected_models: List[str] | None, only_model: str | None) -> set[int] | None:
    raw_names: List[str] = []
    if only_model is not None:
        raw_names.append(only_model)
    if selected_models:
        for value in selected_models:
            raw_names.extend(part.strip() for part in value.split(",") if part.strip())
    if not raw_names:
        return None
    return {find_model_index(model_name) for model_name in raw_names}
...
def obj1_candidate_action_ids(env, agent_idx: int, valid_masks: np.ndarray) -> List[int]:
    agent = env.agents[agent_idx]
...
def obj1_candidate_action_ids_with_support(
    env,
    agent_idx: int,
    valid_masks: np.ndarray,
    persisted_actions: List[int] | None = None,
    ...
    return obj1_candidate_action_ids(env, agent_idx, valid_masks)
...
def build_candidate_views(
    ...
    agent = env.agents[agent_idx]
    candidate_ids = obj1_candidate_action_ids_with_support(
        env,
        agent_idx,
        valid_masks,
        persisted_actions,
...
def build_central_shared_context(
    ...
    shared_rules_lines = [
        ...
        "Use suggested steps when the action is stable enough to execute safely.",
    ]
    return {
        "stated_objective": STATED_OBJECTIVE,
...
def battery_safety_snapshot(agent_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    batteries = [float(row["battery"]) for row in agent_rows]
    agents_below_required = [
        {
        ...
        if float(row["battery"]) < MINIMUM_REQUIRED_CHARGE
    ]
    return {
        "critical_battery_threshold": float(CRITICAL_BATTERY_THRESHOLD),
        "minimum_required_charge": float(MINIMUM_REQUIRED_CHARGE),
        "min_battery_this_step": min(batteries) if batteries else None,
        "agents_below_required_min_charge": agents_below_required,
        "below_required_min_charge_count": len(agents_below_required),
    }
...
def run_single_episode(
    ...
    per_agent_min_battery = [float(agent.battery) for agent in base_env.agents]
    per_agent_below_required_min_charge_steps = [0] * base_env.num_agents
    below_required_min_charge_events = 0
    latest_env_info: Dict[str, Any] | None = None
    ...
    while True:
        ...
        battery_safety = battery_safety_snapshot(post_step_agents)
        below_required_min_charge_events += int(battery_safety["below_required_min_charge_count"])
        for row in post_step_agents:
            agent_id = int(row["agent_id"])
            battery = float(row["battery"])
            per_agent_min_battery[agent_id] = min(float(per_agent_min_battery[agent_id]), battery)
            if battery < MINIMUM_REQUIRED_CHARGE:
                per_agent_below_required_min_charge_steps[agent_id] += 1
    ...
    per_agent_min_battery_summary = {
        f"agent_{idx}": float(value) for idx, value in enumerate(per_agent_min_battery)
    }
    per_agent_below_required_summary = {
        f"agent_{idx}": int(value)
        for idx, value in enumerate(per_agent_below_required_min_charge_steps)
    }
    agents_ever_below_required = [
        f"agent_{idx}"
        for idx, value in enumerate(per_agent_below_required_min_charge_steps)
        if int(value) > 0
    ]
    min_battery_observed = (
        min(float(value) for value in per_agent_min_battery)
        if per_agent_min_battery
        else None
    )
    ...
    summary = {
        ...
        "critical_battery_threshold": float(CRITICAL_BATTERY_THRESHOLD),
        "minimum_required_charge": float(MINIMUM_REQUIRED_CHARGE),
        "min_battery_observed": min_battery_observed,
        "per_agent_min_battery": per_agent_min_battery_summary,
        "below_required_min_charge_events": int(below_required_min_charge_events),
        "per_agent_below_required_min_charge_steps": per_agent_below_required_summary,
        "agents_ever_below_required_min_charge": agents_ever_below_required,
        "battery_safety_success": int(below_required_min_charge_events) == 0,
    ...
    summary_text_lines = [
        ...
        f"Minimum battery observed: {summary['min_battery_observed']}",
        f"Below required minimum charge events: {summary['below_required_min_charge_events']}",
        f"Agents ever below required minimum charge: {', '.join(summary['agents_ever_below_required_min_charge']) if summary['agents_ever_below_required_min_charge'] else 'none'}",
        f"Battery safety success: {summary['battery_safety_success']}",
    ...
    log(
        ...
        f"min_battery={min_battery_observed} below_min_charge_events={below_required_min_charge_events} "
        f"battery_safety_success={summary['battery_safety_success']}",
        log_path,
    )
    return summary
...
def main() -> int:
    ...
    selected_model_idx = selected_model_indices(args.selected_models, args.only_model)
    ...
    for model_idx, (model, model_class) in enumerate(MODELS):
        if selected_model_idx is not None and model_idx not in selected_model_idx:
            continue
