...
A natural question is whether, if language evolved alongside sophisticated forms of collective action and modern language models acquire patterns from language that humans use to describe goals, plans, responsibilities, and constraints, such models could contribute to decision-making in situations where multiple entities must work together towards a shared objective. The present thesis investigates this possibility in the context of multi-agent systems.
...
Multi-robot systems appear in many domains, including manufacturing, transportation, agriculture, exploration, and logistics. Among these, warehouse logistics has received particular attention due to the rapid growth of e-commerce and the increasing demand for fast order fulfilment. According to recent estimates, global retail e-commerce sales exceeded USD 6 trillion in 2024 and continue to grow steadily, while e-commerce activity now represents a significant share of global retail transactions~\cite{statista2025ecommerce,unctad2024digital}. This growth places substantial pressure on warehouse operations, fulfilment networks, and inventory management systems. As a result, improving warehouse efficiency has become both an important industrial objective and an active area of research.
...
To investigate this question systematically, the study examines the influence of three design factors: language-model size, planning architecture, and prompt representation. The following sub-questions are considered:

\begin{enumerate}
\item \textbf{RQ1 (Model Size):} How does language-model size affect collaborative task performance in a constrained warehouse environment, measured through delivery throughput, language-model call efficiency, and observed failure behaviour?

\item \textbf{RQ2 (Planning Architecture):} How do centralized planning and shared-context per-agent planning differ in their ability to support collaborative task performance, measured through delivery throughput, robustness across scenarios with varying collaboration demands, and the prevalence of non-productive or failed runs?

\item \textbf{RQ3 (Prompt Representation):} How do natural-language and \gls{json}-based environment representations affect collaborative task performance, measured through delivery throughput, language-model call efficiency, and the consistency of valid and productive decisions?

\end{enumerate}

The research questions are evaluated in a warehouse-inspired collaborative environment involving heterogeneous agents operating under shared constraints. Performance is assessed using delivery throughput, language-model call efficiency, failure behaviour, and scenario-specific robustness measures.
...
\item Comparative empirical evidence on the effects of model size, planning architecture, and prompt representation in language-model-based collaborative decision-making. The results characterise their impact on throughput, language-model utilisation, robustness, and failure behaviour, while providing insights into the role of grounding and validation mechanisms.
\end{itemize}
